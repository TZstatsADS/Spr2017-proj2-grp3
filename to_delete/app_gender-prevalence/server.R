...
shinyServer(function(input, output){
  ...
  # read the year name
  YearName <- unique(prevalence$year)

  # read the gender type
  GenderType <- unique(prevalence$class)
...
prevalenceDF <- reactive({
  ...
  output$RegPlot<-renderPlotly({
    ...
    if (length(SelectedYear())>0 & length(SelectedGender())>0){
      p <- plot_ly(prevalenceDF(),x=~disease, y=~percentage, color=~class, 
                   type="box", boxpoints = "all", jitter = 0.3) %>%
        layout(title="Disease Prevalence Percentage from All States")
      p
    } 
  })    
})     
